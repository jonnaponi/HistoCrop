...
% Scale the BoundinBox to 1.05 times its original size
scale = 1.05;

% Adjust the lower left corner of the rectangles
spots(:,2:3) = spots(:,2:3) - spots(:,4:5)*0.5*(scale - 1);

% Adjust the width and height of the rectangles
spots(:,4:5) = spots(:,4:5)*scale;
...
for i=1:length(spots)
    summaryImg = insertText(summaryImg,[spots(i,2)+1,spots(i,3)+1],...
        new_spot_nums(i),'BoxOpacity',0,'TextColor','black','FontSize',32);
end
end
