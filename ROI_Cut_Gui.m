function varargout = ROI_Cut_Gui(varargin)
% ROI_CUT_GUI MATLAB code for ROI_Cut_Gui.fig
%      ROI_CUT_GUI, by itself, creates a new ROI_CUT_GUI or raises the existing
%      singleton*.
%
%      H = ROI_CUT_GUI returns the handle to a new ROI_CUT_GUI or the handle to
%      the existing singleton*.
%
%      ROI_CUT_GUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ROI_CUT_GUI.M with the given input arguments.
%
%      ROI_CUT_GUI('Property','Value',...) creates a new ROI_CUT_GUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ROI_Cut_Gui_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ROI_Cut_Gui_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ROI_Cut_Gui

% Last Modified by GUIDE v2.5 23-Sep-2019 17:24:55

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @ROI_Cut_Gui_OpeningFcn, ...
    'gui_OutputFcn',  @ROI_Cut_Gui_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before ROI_Cut_Gui is made visible.
function ROI_Cut_Gui_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ROI_Cut_Gui (see VARARGIN)

% Choose default command line output for roi_cut_gui
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);
% UIWAIT makes roi_cut_gui wait for user response (see UIRESUME)
...
function varargout = ROI_Cut_Gui_OutputFcn(hObject, eventdata, handles)
...
%Initial image
path=pwd;
I=imread([path filesep 'ROI_Cover.png']);
imshow(I)
...
save('List_Rect','add_rect_data','add_Img_num');
